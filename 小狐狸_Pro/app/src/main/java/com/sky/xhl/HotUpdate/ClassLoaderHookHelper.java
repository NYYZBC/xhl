...
public class ClassLoaderHookHelper {
    ...
    public static void hookV23(ClassLoader classLoader, File outDexFilePath, File optimizedDirectory) throws IllegalAccessException, InvocationTargetException {
        ...
        Method makePathElementsMethod = ReflectionUtil.getMethod(dexPathListObj, "makePathElements", List.class, File.class, List.class);
        ...
        if (newElements != null && newElements.length > 0) {
            ...
            System.arraycopy(newElements, 0, dexElements, 0, newElements.length);//新来的数组放前面
            System.arraycopy(oldElements, 0, dexElements, newElements.length, oldElements.length);
        }
    ...
    public static void hookV19(ClassLoader classLoader, File outDexFilePath, File optimizedDirectory) throws IllegalAccessException, InvocationTargetException {
        ...
        Object[] newElements = (Object[]) makePathElementsMethod.invoke(null, files, optimizedDirectory, ioExceptions);// 这个方法是静态方法，所以不需要传实例,直接invoke;这里取得的返回值就是 我们外部的dex文件构建成的 Element数组
